...
const state={config:null,rounds:[],round:0,lives:3,score:0,correct:0,mistakes:0,wrongThisRound:false,locked:true,sfx:true,currentWord:null,audio:null,timers:[]};
...
function prepareGame(){const cfg=state.config;state.rounds=shuffle(cfg.pairs).map(pair=>{const target=pair.options[Math.floor(Math.random()*2)];return{pair,options:shuffle(pair.options),target}});state.round=0;state.lives=cfg.startingLives;state.score=0;state.correct=0;state.mistakes=0;state.wrongThisRound=false;state.locked=true;state.journey=null;$("#score").textContent="0"}
...
function renderLives(){const box=$("#lives");box.innerHTML="";for(let i=0;i<state.config.startingLives;i++){const img=document.createElement("img");img.className="life";img.src=i<state.lives?state.config.ui.crystalWhole:state.config.ui.crystalLost;img.alt=i<state.lives?"Целый кристалл":"Потерянный кристалл";box.append(img)}}
...
async function correct(button){state.locked=true;state.score++;state.correct++;$("#score").textContent=state.score;window.MirProgress?.correct('forest');$$('.portal').forEach(b=>b.disabled=true);setFrame(button,state.config.portals.selected);await sleep(175);setFrame(button,state.config.portals.correct);$("#journeyYashka").src=state.config.yashka.victory;play(state.config.sounds.correct,{volume:.48});reward(button);renderLives();setSprites(state.config.yashka.runRight,state.config.fog.blownBack);await sleep(1000);state.round++;if(state.round>=state.config.roundsPerGame)return victory();startRound()}
...
async function wrong(button){
  ...
  if(state.lives===0){
    await sleep(650);
    hideGameCharacters();
    $("#defeatScore").textContent=state.score;
    $("#defeatCorrect").textContent=state.correct;
...
async function victory(){state.locked=true;hideGameCharacters();$("#finalScore").textContent=state.score;$("#finalCorrect").textContent=state.correct;showScreen('victory');window.MirProgress?.finish('forest',{score:state.score,completed:true});await play(state.config.sounds.victory,{volume:.5})}
async function begin(){window.MirProgress?.startAttempt('forest');state.locked=true;$("#loadingStatus").textContent="Загружаем лес…";showScreen('game');$("#loadingStatus").classList.add('visible');await preload();prepareGame();$("#loadingStatus").textContent="Слушаем…";startRound()}
...
function toggleSound(){state.sfx=!state.sfx;[$("#sfxToggle"),$("#startSound")].filter(Boolean).forEach(button=>{button.textContent="♫";button.setAttribute('aria-pressed',String(!state.sfx));button.setAttribute('aria-label',state.sfx?'Выключить игровые звуки':'Включить игровые звуки');button.style.opacity=state.sfx?'1':'.55'})}
$("#startButton").addEventListener('click',begin);$("#repeatButton").addEventListener('click',()=>{if(state.currentWord)play(state.currentWord.audio,{speech:true})});$$('.open-help').forEach(b=>b.addEventListener('click',openHelp));$("#closeHelp").addEventListener('click',closeHelp);$("#helpModal .modal-shade").addEventListener('click',closeHelp);document.addEventListener('keydown',e=>{if(e.key==='Escape'&&!$("#helpModal").hidden)closeHelp()});$("#retryButton").addEventListener('click',()=>{window.MirProgress?.startAttempt('forest');$("#defeatModal").hidden=true;prepareGame();startRound()});$("#playAgain").addEventListener('click',()=>{window.MirProgress?.startAttempt('forest');showScreen('game');prepareGame();startRound()});$$('.to-menu').forEach(b=>b.addEventListener('click',menu));[$("#sfxToggle"),$("#startSound")].filter(Boolean).forEach(button=>button.addEventListener('click',toggleSound));
